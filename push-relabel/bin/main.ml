...
module Map =
 ...
 struct
  ...
  | p :: xs0 ->
    let (u, y) = p in
    if T__8.eqb v u then remove d v xs0 else (u, y) :: (remove d v xs0)

  (** val replace :
      'a1 -> T__8.coq_V -> 'a1 -> 'a1 t -> (T__8.coq_V * 'a1) list **)
  ...
  (** val update :
      'a1 -> T__8.coq_V -> ('a1 -> 'a1) -> 'a1 t -> (T__8.coq_V * 'a1) list **)
  ...
  let rec find d xs v =
    match xs with
    | [] -> d
    | p :: xs0 -> let (u, y) = p in if T__8.eqb v u then y else find d xs0 v
 end

module MkSet =
 functor (T__10:T) ->
 struct
  type t = T__10.coq_V list
  ...
  (** val remove : T__10.coq_V -> T__10.coq_V list -> T__10.coq_V list **)

  let rec remove v = function
  | [] -> []
  | v' :: s0 -> if T__10.eqb v v' then remove v s0 else v' :: (remove v s0)

  (** val add : T__10.coq_V -> T__10.coq_V list -> T__10.coq_V list **)

  let add v s =
    v :: (remove v s)

  (** val mem : T__10.coq_V -> T__10.coq_V list -> bool **)

  let rec mem v = function
  | [] -> false
  | v' :: s0 -> if T__10.eqb v v' then true else mem v s0

  (** val choice : T__10.coq_V list -> (T__10.coq_V * t) option **)

  let choice = function
  | [] -> None
  | v :: s0 -> Some (v, s0)

  (** val filter : (T__10.coq_V -> bool) -> t -> T__10.coq_V list **)

  let rec filter p = function
  | [] -> []
  | v :: s -> if p v then v :: (filter p s) else filter p s

  (** val fold_left : ('a1 -> T__10.coq_V -> 'a1) -> t -> 'a1 -> 'a1 **)
...
module Tuple =
 ...
 functor (U:T) ->
 struct
  type coq_V = T__11.coq_V * U.coq_V

  (** val eqb : (T__11.coq_V * U.coq_V) -> (T__11.coq_V * U.coq_V) -> bool **)

  let eqb pat pat0 =
    let (a, b) = pat in let (c, d) = pat0 in (&&) (T__11.eqb a c) (U.eqb b d)
...
module PR =
 ...
 struct
  ...
  module Edge = Tuple(T__13)(T__13)

  module EMap = Map(Edge)

  module VSet = MkSet(T__13)
  ...
  type coq_FlowNet =
    ((coq_Graph * (T__13.coq_V -> T__13.coq_V ->
    coq_R)) * T__13.coq_V) * T__13.coq_V
  ...
  (** val sink : coq_FlowNet -> T__13.coq_V **)

  let sink = function
  | (_, t0) -> t0

  (** val source : coq_FlowNet -> T__13.coq_V **)
  ...
  (** val excess : coq_FlowNet -> coq_R EMap.t -> T__13.coq_V -> coq_R **)
  ...
  (** val res_cap :
      coq_FlowNet -> coq_R EMap.t -> T__13.coq_V -> T__13.coq_V -> coq_R **)
  ...
  module NMap = Map(T__13)

  (** val push :
      ((((VSet.t * ESet.t) * (T__13.coq_V -> T__13.coq_V ->
      coq_R)) * T__13.coq_V) * T__13.coq_V) -> coq_R EMap.t -> T__13.coq_V ->
      T__13.coq_V -> coq_R EMap.t **)
  ...
  (** val relabel_find :
      coq_FlowNet -> coq_R EMap.t -> int NMap.t -> T__13.coq_V -> VSet.t ->
      T__13.coq_V option **)
  ...
  (** val relabel :
      ((((VSet.t * ESet.t) * (T__13.coq_V -> T__13.coq_V ->
      coq_R)) * T__13.coq_V) * T__13.coq_V) -> coq_R EMap.t -> int NMap.t ->
      T__13.coq_V -> int NMap.t option **)
  ...
  (** val find_push_node :
      ((((VSet.t * ESet.t) * (T__13.coq_V -> T__13.coq_V ->
      coq_R)) * T__13.coq_V) * T__13.coq_V) -> coq_R EMap.t -> int NMap.t ->
      T__13.coq_V -> T__13.coq_V list -> T__13.coq_V option **)
  ...
  let has_excess_not_sink fn f v =
    let (y, t0) = fn in
    let (_, s) = y in
    if (||) (T__13.eqb v t0) (T__13.eqb v s)
    then false
    else coq_QLt (0, 1) (excess fn f v)

  type coq_Tr =
  | Init of (int * int) * (int * int) EMap.t * int NMap.t * VSet.t
  | Push of (int * int) * T__13.coq_V * T__13.coq_V * (int * int) EMap.t
     * VSet.t
  | Relabel of T__13.coq_V * int * int NMap.t
  | OutOfGas
  | RelabelFailed

  (** val coq_Tr_rect :
      ((int * int) -> (int * int) EMap.t -> int NMap.t -> VSet.t -> 'a1) ->
      ((int * int) -> T__13.coq_V -> T__13.coq_V -> (int * int) EMap.t ->
      VSet.t -> 'a1) -> (T__13.coq_V -> int -> int NMap.t -> 'a1) -> 'a1 ->
      'a1 -> coq_Tr -> 'a1 **)
  ...
  (** val gpr_helper_trace :
      ((((VSet.t * ESet.t) * (T__13.coq_V -> T__13.coq_V ->
      coq_R)) * T__13.coq_V) * T__13.coq_V) -> coq_R EMap.t -> int NMap.t ->
      T__13.coq_V list -> int -> coq_Tr list -> ((int * int) EMap.t * int
  ...
  (** val initial_push :
      ((((VSet.t * ESet.t) * (T__13.coq_V -> T__13.coq_V ->
      (int * int))) * T__13.coq_V) * T__13.coq_V) -> (int * int) EMap.t ->
      T__13.coq_V list -> (T__13.coq_V * T__13.coq_V) list -> (int * int)
      EMap.t * T__13.coq_V list **)
